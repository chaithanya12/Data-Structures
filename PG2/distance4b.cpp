 ...
 *		> calculates, sorts, then prints the distance from the user-inputted latitude and longitude to each location stored in the vector/list
 */
...
int main(int argc, char**argv){
	...
	Distance<list<Location> > my_Distance;
	...
	// copy into vector
	Distance<vector<Location> > my_Distance2;
	my_Distance2 = my_Distance;
	
	// calculate the distance from each point in vector/list to the user location
	my_Distance2.calc_dist_from(user_loc);
	// sort the distances from each location
	my_Distance2.sort_by_dist();

	// copy back into list and then print
	my_Distance = my_Distance2;
